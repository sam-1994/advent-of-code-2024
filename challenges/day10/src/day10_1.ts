...
function findTrails(
  ...
  if (0 < x && map[y][x - 1] === currentNumber + 1) {
    const leftTrails = findTrails(map, x - 1, y, currentNumber + 1, visitedMap).map(
      (trail) => [{ x, y }, ...trail]
    );
  ...
  if (0 < y && map[y - 1][x] === currentNumber + 1) {
    const topTrails = findTrails(map, x, y - 1, currentNumber + 1, visitedMap).map(
      (trail) => [{ x, y }, ...trail]
    );
  ...
  if (x < map[y].length - 1 && map[y][x + 1] === currentNumber + 1) {
    const rightTrails = findTrails(map, x + 1, y, currentNumber + 1, visitedMap).map(
      (trail) => [{ x, y }, ...trail]
    );
  ...
  if (y < map.length - 1 && map[y + 1][x] === currentNumber + 1) {
    const bottomTrails = findTrails(map, x, y + 1, currentNumber + 1, visitedMap).map(
      (trail) => [{ x, y }, ...trail]
    );

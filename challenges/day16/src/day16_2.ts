...
export function solve(input: string): number {
  ...
  const paths = searchEnd(map, reindeer, end).map((state) => state.path);

  const allPositions: Position[] = paths.reduce((acc, path) => {
    path.forEach((position) => {
      if (!acc.some((pos) => pos.x === position.x && pos.y === position.y)) {
        acc.push(position);
      }
    });
    return acc;
